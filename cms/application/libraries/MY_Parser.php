...
class MY_Parser extends CI_Parser {
    ...
    public function __construct()
    {
        ...
        if ($this->_theme_name == '')
        {
            $this->set_theme($this->CI->config->item('theme_name'));
        }
    ...
    */
    ...
    public function set_theme($name)
    {
        ...
        $functions_file  = $this->CI->config->item('theme_path') . $this->_theme_name . '/functions.php';
     ...
     * Partial
     *
     * Parses a content template using Smarty 3 engine
     *
     * @access public
     * @param $template
     * @param $data
     ...
     * @return string
     */
    public function set_partial($part = null,$template, $data = array(), $return = FALSE, $caching = TRUE)
    {
        // If we don't want caching, disable it
        if ( $caching === FALSE )
        {
        ...
        if($template != '')
        {
            ...
            if ( ! stripos($template, ':') )
            {
                $template = $this->_find_partial($template);
            }
        ...
        */
        ...
        $this->_partial[$part] = $template_string;
     ...
     * Get Partial
     *
     * Does what the function name implies: gets the partial partial
     * to use in theme.
     *
     * @return string
     */
    ...
    {
        return (isset($this->_partial[$part])) ? $this->_partial[$part] : '';
    }

    
    public function get_partial_path($part)
    {
        return (isset($this->_partial[$part])) ? $this->_partial[$part] : '';
    }
    ...
    public function css($file, $attributes = array())
    {
        ...
        $return = '<link rel="'.$attributes['rel'].'" type="'.$attributes['type'].'" href="'.base_url($this->CI->config->item('theme_path').$this->get_theme()."/css/".$file).'" media="'.$attributes['media'].'">';
    ...
    public function js($file, $attributes = array())
    {
        ...
        $return = '<script type="'.$attributes['type'].'" src="'.base_url($this->CI->config->item('theme_path').$this->get_theme()."/js/".$file).'"></script>';
    ...
    public function img($file, $attributes = array())
    {
        ...
        $return = '<img src ="'.base_url($this->CI->config->item('theme_path').$this->get_theme()."/img/".$file).'" alt="'.$attributes['alt'].'" title="'.$attributes['title'].'" />';
    ...
    {
        // The path to return
        $return = base_url($this->CI->config->item('theme_path').$this->get_theme())."/";
    ...
    */
    protected function _find_view($file)
    {
        ...
        {
            $new_locations = array(
                $this->CI->config->item('theme_path') . $this->_theme_name . '/views/modules/' . $current_module .'/layouts/',
                $this->CI->config->item('theme_path') . $this->_theme_name . '/views/modules/' . $current_module .'/',
                $this->CI->config->item('theme_path') . $this->_theme_name .'/',
    ...
    * Find Partial
    *
    * Searches through module and view folders looking for your partial view, sir.
    *
    * @access protected
    * @param $file
    * @return string The path and file found
    */
    protected function _find_partial($file)
    {
        // We have no path by default
        $path = NULL;
    ...
    */
    ...
    protected function _update_theme_paths()
    {
        ...
        $this->_template_locations = array(
            $this->CI->config->item('theme_path') . $this->_theme_name . '/modules/' . $this->_module .'/layouts/',
            $this->CI->config->item('theme_path') . $this->_theme_name . '/modules/' . $this->_module .'/',
            $this->CI->config->item('theme_path') . $this->_theme_name . '/layouts/',
            $this->CI->config->item('theme_path') . $this->_theme_name .'/',
            //$this->CI->config->item('theme_path') . $this->_theme_name . '/views/',
