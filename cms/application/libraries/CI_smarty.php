...
class CI_Smarty extends Smarty {
    ...
    public function __construct()
    {
        ...
        // Turn on/off debug
        $this->debugging = $CI->config->item('smarty_debug');

        // Set some pretty standard Smarty directories
        $this->setCompileDir($CI->config->item('compile_directory'));
        $this->setCacheDir($CI->config->item('cache_directory'));
        $this->setConfigDir($CI->config->item('config_directory'));
        ...
        if ( $CI->config->item('cache_status') === TRUE )
        {
            $this->enable_caching();
        }
        else
        ...
        $this->error_reporting   = $CI->config->item('template_error_reporting');
